...
#for N in 8640000 9720000 10800000; do 
for N in 8640000 10800000; do 
    for two_flow in 1; do

        if [ $N -eq 8640000 ]; then
            NB=4000
        elif [ $N -eq 9720000 ]; then
            NB=4500
        elif [ $N -eq 10800000 ]; then
            NB=4500
        fi
        ...
        for loop in `seq 0 $NO`; do
            cmd="$sruncmd \
                 ...
                 $home/testing_dpotrf -P $p -Q $q -N $N -t $NB -e 1e-8 --maxrank $maxrank -j $N -v -c $core --genmaxrank $genmaxrank --compmaxrank $compmaxrank -D 4 -Z 1 -F 0 -Y 1 -z 300 -E 1 -W 1"
            echo $cmd
