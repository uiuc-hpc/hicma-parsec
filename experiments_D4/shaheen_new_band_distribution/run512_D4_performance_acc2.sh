...
filename=shaheen_D4_performance_nodes"$nodes"_acc2
...
for N in `seq 1080000 1080000 8640000`; do
    for two_flow in 1; do
        ...
        if [ $N -eq 1080000 ]; then
            ...
            NB=4500
        elif [ $N -eq 8640000 ]; then
            NB=4500
        elif [ $N -eq 9720000 ]; then
            NB=5000
        fi
        ...
        for loop in `seq 0 $NO`; do
            cmd="$sruncmd \
                 ...
                 $home/testing_dpotrf -P $p -Q $q -N $N -t $NB -e 1e-2 --maxrank $maxrank -j $N -v -c $core --genmaxrank $genmaxrank --compmaxrank $compmaxrank -D 4 -Z 1 -F 0 -Y 1 -z 300 -E 1 -W $two_flow"
            echo $cmd
            eval $cmd 2>&1 | tee tmp_$filename.txt

            # Check memory
	    cat tmp_$filename.txt | grep memory_for_matrix_allocation_per_node
	    if [ $? -eq 0 ]; then
                echo "N $N NB $NB: out_of_memory"
	        exit 1	
	    fi

            all_put=`cat tmp_$filename.txt | grep R-LO`
            #echo $all_put
