...
filename=shaheen_D4_performance_nodes"$nodes"_only_band
...
#for N in 9720000; do 
    for two_flow in 1; do
        ...
        for loop in `seq 0 $NO`; do
            ...
            echo $cmd
            eval $cmd 2>&1 | tee tmp_only_band_$nodes.txt

            # Check memory
	    cat tmp_only_band_$nodes.txt | grep memory_for_matrix_allocation_per_node
	    if [ $? -eq 0 ]; then
                echo "N $N NB $NB: out_of_memory"
	        exit 1	
	    fi

            cat tmp_only_band_$nodes.txt >> result/shaheen_D4_performance_all.csv
            all_put=`cat tmp_only_band_$nodes.txt | grep R-LO`
            #echo $all_put
