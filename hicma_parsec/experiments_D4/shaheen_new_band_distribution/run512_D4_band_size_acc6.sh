...
for N in 1080000; do
  ...
  file_name=shaheen_"$nodes"node_band_size_acc6.txt
  #for band_size in `seq 1 17`; do
  for band_size in 11; do
    cmd="$sruncmd \
         numactl --interleave=all \
         $home/testing_dpotrf -P $P -Q $Q -N $N -t $NB -e 1e-6 --maxrank $maxrank -j $N -v -c 31 --genmaxrank $maxrank --compmaxrank $maxrank -D 4 --band $band_size -F 0 -Y 1 -z 300 -E 0" 
    echo $cmd
