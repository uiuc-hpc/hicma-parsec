...
filename=shaheen_D4_performance_nodes"$nodes"_acc6
	...
	if [ $genmaxrank -lt 500 ]; then
        ...
        for loop in `seq 0 $NO`; do
            cmd="$sruncmd \
                 ...
                 $home/testing_dpotrf -P $p -Q $q -N $N -t $NB -e 1e-6 --maxrank $maxrank -j $N -v -c $core --genmaxrank $genmaxrank --compmaxrank $compmaxrank -D 4 -Z 1 -F 0 -Y 1 -z 300 -E 1 -W $two_flow"
            echo $cmd
