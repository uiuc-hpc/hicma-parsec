...
#SBATCH --output=/project/k1205/lei/lorapo/experiments/D1w50/out/%j
#SBATCH --error=/project/k1205/lei/lorapo/experiments/D1w50/out/%j
...
numnodes=64; nummpi=64; ntasks_per_node=1; nthread=32; p=8; q=8
core=31
...
for opt_combine in "--band 0 -F 1 -Y 0" "--band 1 -F 1 -Y 0" "--band 1 -F 0 -Y 0" "--band 1 -F 0 -Y 1" "--band 1 -F 0 -Y 1"; do
  ...
  # First run
for N in 1080000 `seq 2160000 2160000 8640000`; do
    time_opt=1000000000
    # All 
    ...
    for NB in 1800 2400 2700 3375 3600 4500 5400 7200 9000 10800; do
        ...
        if [ $count -eq 5 ]; then
            ...
            if [ $(bc <<< "$memory_thd < $memory") -eq 1 ]; then
                ...
                if [[ $time_opt != 1000000000 ]]; then
                    ...
                    echo "$N $time_opt $mb_opt $var out_of_memory" >> result/"node$numnodes"_"D$D"_"w$w"_"$count"
                fi
                continue
            fi
            ...
            for loop in `seq 0 $NO`; do
                cmd="$sruncmd \
                     ...
                     $home/testing_dpotrf -P $p -Q $q -N $N -t $NB -e 1e-8 --maxrank $maxrank -j $N -v -c $core --genmaxrank $genmaxrank --compmaxrank $compmaxrank -D $D -w $w $opt_combine -z $subtile"
                echo $cmd
                all_put=`eval $cmd | grep R-LO`
                echo $all_put
                echo $all_put >> all_results_"D$D"_"w$w"
                out[$loop]=`echo $all_put | awk '{printf $28}'`
                echo "$N ${out[$loop]} $NB $var" >> result/"node$numnodes"_"D$D"_"w$w"_"$count"_all
            ...
            if [[ ${out[0]} != "" ]]; then
                if [ $(bc <<< "${out[0]} < $time_opt") -eq 1 ]; then
                   ...
                   echo "$N $time_opt $mb_opt $var" >> result/"node$numnodes"_"D$D"_"w$w"_"$count"
                   echo ""
                   break 
                fi
                ...
                if [ $NB -eq $NB_max ]; then
                   ...
                   echo "$N $time_opt $mb_opt $var not_reach_bottom" >> result/"node$numnodes"_"D$D"_"w$w"_"$count"
                   echo ""
                fi
            ...
            if [[ $NB == $NB_max && $time_opt != 1000000000 ]]; then
                ...
                echo "$N $time_opt $mb_opt $var only_one_got" >> result/"node$numnodes"_"D$D"_"w$w"_"$count"
                echo ""
            fi
