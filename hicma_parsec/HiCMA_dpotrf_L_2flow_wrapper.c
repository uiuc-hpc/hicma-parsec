...
static int syrk_id = 6;
static int gemm_id = 7;
...
static float evaluate_gpu_trsm(parsec_task_t* task) {
    int m = ((__parsec_HiCMA_dpotrf_L_2flow_potrf_dtrsm_task_t *)task)->locals.m.value;
    int k = ((__parsec_HiCMA_dpotrf_L_2flow_potrf_dtrsm_task_t *)task)->locals.k.value;
    int band_size = ((__parsec_HiCMA_dpotrf_L_2flow_potrf_dtrsm_task_t *)task)->locals.band_size_local.value;
    if( m-k < band_size )
        return PARSEC_HOOK_RETURN_DONE;
    else
...
static float evaluate_gpu_gemm(parsec_task_t* task) {
    int m = ((__parsec_HiCMA_dpotrf_L_2flow_potrf_dgemm_task_t *)task)->locals.m.value;
    int n = ((__parsec_HiCMA_dpotrf_L_2flow_potrf_dgemm_task_t *)task)->locals.n.value;
    int k = ((__parsec_HiCMA_dpotrf_L_2flow_potrf_dgemm_task_t *)task)->locals.k.value;
    int band_size = ((__parsec_HiCMA_dpotrf_L_2flow_potrf_dgemm_task_t *)task)->locals.band_size_local.value;
    int send_full = ((__parsec_HiCMA_dpotrf_L_2flow_potrf_dgemm_task_t *)task)->locals.send_full_tile_local.value;
    if( m-n < band_size )
        return PARSEC_HOOK_RETURN_DONE;
    else
        return PARSEC_HOOK_RETURN_NEXT;
                  ...
                  int *info )
{
    ...
    if ((uplo == PlasmaUpper)) {
        dplasma_error("HiCMA_dpotrf_L_2flow_New", "HiCMA_dpotrf_L_2flow does not support PlasmaUpper for now");
        return NULL /*-1*/;
    }
    ...
    *info = 0;
    if ( uplo == PlasmaLower ) {
        parsec_HiCMA_dpotrf_L_2flow_taskpool_t *hicma_dpotrf =
            parsec_HiCMA_dpotrf_L_2flow_new( uplo, A, Ar, RG, Rank, info,
